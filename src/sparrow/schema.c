...
void sparrow_schema_release_internal(struct ArrowSchema* schema) {
  ...
  if (schema->children != NULL) {
    for (uint64_t i = 0; i < schema->n_children; i++) {
      sparrow_schema_release_internal(schema->children[i]);
  ...
  if (schema->dictionary != NULL) {
    sparrow_schema_release_internal(schema->dictionary);
...
int64_t sparrow_schema_metadata_size(const char* metadata) {
  if (metadata == NULL) {
    return 0;
  }
...
void sparrow_schema_reset_internal(struct ArrowSchema* schema) {
  schema->format = NULL;
  schema->name = NULL;
  ...
  schema->release = &sparrow_schema_release_internal;
}

int sparrow_schema_deep_copy(struct ArrowSchema* out, struct ArrowSchema* schema) {
  if (out == NULL || schema == NULL) {
    return EINVAL;
  }

  sparrow_schema_reset_internal(out);
  ...
  int64_t metadata_size = sparrow_schema_metadata_size(schema->metadata);
  if (metadata_size > 0) {
  ...
  if (out->n_children > 0) {
    ...
    for (int64_t i = 0; i < out->n_children; i++) {
      ...
      result = sparrow_schema_deep_copy(out->children[i], schema->children[i]);
      if (result != 0) {
        out->release(out);
        return result;
      }
  ...
  if (schema->dictionary != NULL) {
    ...
    result = sparrow_schema_deep_copy(out->dictionary, schema->dictionary);
    if (result != 0) {
      out->release(out);
      return result;
    }
