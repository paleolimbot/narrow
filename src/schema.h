...
#ifndef sparrow_SCHEMA_H_INCLUDED
#define sparrow_SCHEMA_H_INCLUDED
...
static inline SEXP schema_xptr_new(struct ArrowSchema* schema) {
  ...
  Rf_setAttrib(schema_xptr, R_ClassSymbol, Rf_mkString("sparrow_schema"));
  UNPROTECT(1);
  return schema_xptr;
}
...
static inline void schema_export(SEXP schema_xptr, struct ArrowSchema* schema_copy) {
  ...
  sparrow_schema_deep_copy(schema_copy, schema);
}
