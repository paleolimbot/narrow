...
#ifndef narrow_int64_H_INCLUDED
#define narrow_int64_H_INCLUDED
...
static inline int64_t scalar_int64_from_sexp(SEXP int64_sexp, const char* arg) {
  if (Rf_inherits(int64_sexp, "narrow_int64")) {
    int64_t out;
    ...
    Rf_error("`%s` must be numeric(1), integer(1), or narrow_int64(1)", arg);
  }
}
