...
int carrow_vector_alloc_buffers(struct ArrowVector* vector, int32_t which_buffers, struct ArrowStatus* status) {
  ...
  int has_existing_offset_buffer = carrow_vector_offset_buffer(vector) != NULL;
  int has_existing_large_offset_buffer = carrow_vector_large_offset_buffer(vector) != NULL;
  ...
  if (which_buffers & CARROW_BUFFER_OFFSET) {
    int32_t* offset_buffer = carrow_vector_offset_buffer(vector);
    ...
    int64_t* large_offset_buffer = carrow_vector_large_offset_buffer(vector);
  ...
  if (which_buffers & CARROW_BUFFER_UNION_TYPE) {
    char* union_type_buffer = carrow_vector_union_type_buffer(vector);
  ...
  if (which_buffers & CARROW_BUFFER_CHILD) {
    ...
    for (int64_t i = 0; i < vector->schema->n_children; i++) {
      carrow_vector_init(&child_vector, vector->schema->children[i], vector->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);

      carrow_vector_alloc_buffers(&child_vector, which_buffers, status);
      RETURN_IF_NOT_OK(status);
    }
  }
  ...
  if (which_buffers & CARROW_BUFFER_DICTIONARY) {
    ...
    if (vector->schema->dictionary != NULL) {
      carrow_vector_init(&child_vector, vector->schema->dictionary, vector->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);
