...
int carrow_array_copy(struct CarrowArray* array_dst, int64_t dst_offset,
  ...
  if (array_dst == NULL) {
    carrow_status_set_error(status, EINVAL, "`array_dst` is NULL");
    RETURN_IF_NOT_OK(status);
  }

  if (array_src == NULL) {
    carrow_status_set_error(status, EINVAL, "`array_src` is NULL");
    RETURN_IF_NOT_OK(status);
  }

  // dense unions aren't supported yet
  if (array_src->type == CARROW_TYPE_DENSE_UNION) {
    carrow_status_set_error(status, EINVAL, "Dense unions are not supported by carrow_array_copy()");
    RETURN_IF_NOT_OK(status);
  }
  ...
  unsigned char* validity_buffer_src = carrow_array_validity_buffer(array_src);
  unsigned char* validity_buffer_dst = carrow_array_validity_buffer(array_dst);

  int32_t* offset_buffer_src = carrow_array_offset_buffer(array_src);
  int32_t* offset_buffer_dst = carrow_array_offset_buffer(array_dst);

  int64_t* large_offset_buffer_src = carrow_array_large_offset_buffer(array_src);
  int64_t* large_offset_buffer_dst = carrow_array_large_offset_buffer(array_dst);

  char* union_type_buffer_src = carrow_array_union_type_buffer(array_src);
  char* union_type_buffer_dst = carrow_array_union_type_buffer(array_dst);

  void* data_buffer_src = carrow_array_data_buffer(array_src);
  void* data_buffer_dst = carrow_array_data_buffer(array_dst);
  ...
  } else if (array_src->type == CARROW_TYPE_FIXED_SIZE_LIST) {
  ...
  struct CarrowArray child_array_src;
  struct CarrowArray child_array_dst;

  if (which_buffers & CARROW_BUFFER_CHILD) {
    // copy child arrays
    for (int64_t i = 0; i < array_src->schema->n_children; i++) {
      carrow_array_init(&child_array_src, array_src->schema->children[i], array_src->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);

      carrow_array_init(&child_array_dst, array_dst->schema->children[i], array_dst->array_data->children[i], status);
      RETURN_IF_NOT_OK(status);
  ...
  if (which_buffers & CARROW_BUFFER_DICTIONARY) {
    ...
    if (array_src->schema->dictionary != NULL) {
      carrow_array_init(&child_array_src, array_src->schema->dictionary, array_src->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);

      carrow_array_init(&child_array_dst, array_src->schema->dictionary, array_dst->array_data->dictionary, status);
      RETURN_IF_NOT_OK(status);

      carrow_array_copy(&child_array_dst, 0, &child_array_src, 0, child_array_src.array_data->length, which_buffers, status);
      RETURN_IF_NOT_OK(status);
    }
  }
